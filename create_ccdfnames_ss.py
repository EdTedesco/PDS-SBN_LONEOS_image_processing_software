# -*- coding: utf-8 -*-
"""
Program create_ccdfnames_ss.py
Final version created on Thu 24 July 2025
@author: Ed Tedesco

Purpose: To extract filenames from lois_none (f-formatted) images recovered from
         tape for ALL *.fits files with single-spaced headers in the executing
         directory.

Input:  Recovered image files in format f1234567.fits, f12345678.fits, and/or
        f123456789.fits in the same directory as create_ccdfnames_ss.py and
        images with single-spaced headers containing keywords DATE-UT on line 45
...
Output: Renames the input f-formatted images with the filenames required by the
        lois_none single-spaced pipeline: create_pds_files_none_ss.sh.

        renamed_ss_lois_none_images.txt - A file with the recovered lois_none
        single-spaced filenames and the filenames constructed from their headers.
        This allows each lois_none image to be traced back to its f-formatted
        source image.

Note: This variant is ONLY for lois_none f-formated images with single-spaced
      headers.
...
#           The next line reads the DATE-UT keyword on header line 32 DATE_UT_startline            
...
#
            with open('renamed_ss_lois_none_images.txt', mode='a') as file_object:
